...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('join', function(userType, roomname, callback) {
    ...
    if (userType == 'sender') {
      ...
      if (io.sockets.connected[ROOMINFO[roomname].host]) {
        io.sockets.connected[ROOMINFO[roomname].host].emit('requestConnectRequest', socket.id);
      }
      else {
        io.sockets.connected[socket.id].emit('message', 'host not find');
      }
  ...
  socket.on('disconnect', function() {
    ...
    if (
        ...
        ROOMINFO[currentroom].client.length == 0 &&
        !ROOMINFO[currentroom].host
    ){
