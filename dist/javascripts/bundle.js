...
/***/ function(module, exports, __webpack_require__) {
	...
	var Jrtc = function () {
	  ...
	  _createClass(Jrtc, [{
	    ...
	    value: function requestCreateRtc() {
	    ...
	    key: 'sendStart',
	    value: function sendStart(stream) {
	      ...
	      that.ws.on('requestConnectRequest', function (id) {
	        ...
	        that.parterId = id;
	        that.peer.createOffer().then(that.onCreateOfferSuccess.bind(that), that.onCreateSessionDescriptionError);
	      });

	      that.ws.on('receiveAnswer', function (desc) {
	        console.log("ANSWSERDESC --", desc);
	        that.peer.setRemoteDescription(desc).then(that.onSetRemoteSuccess.bind(that), that.onSetSessionDescriptionError);
	      });

	      that.ws.on('receiveIceCandidate', function (candidate) {
	        that.peer.addIceCandidate(new RTCIceCandidate(candidate)).then(that.onAddIceCandidateSuccess, that.onAddIceCandidateError);
	      });
	    }
	  }, {
	    ...
	    value: function onCreateOfferSuccess(desc) {
	      ...
	      that.peer.setLocalDescription(desc).then(function () {
	    ...
	    value: function onCreateAnswerSuccess(desc) {
	      ...
	      this.peer.setLocalDescription(desc).then(function () {
	        _this2.onSetLocalSuccess(desc);
	    ...
	    value: function receiveChannelCallback(event) {
	    ...
	    value: function onSetLocalSuccess(desc) {
	    ...
	    value: function onSetRemoteSuccess() {
	      ...
	      if (this.type == 'receiver') {
	        this.peer.createAnswer().then(this.onCreateAnswerSuccess.bind(this), this.onCreateSessionDescriptionError);
	      }
	    }
	  }, {
	    key: 'onIceStateChange',
	    value: function onIceStateChange(event) {
