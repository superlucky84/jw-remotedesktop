...
/***/ function(module, exports, __webpack_require__) {
	...
	var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); /* IMPORT WEBRTC */


	var _webrtcAdapter = __webpack_require__(1);
	  ...
	  _createClass(Jrtc, [{
	    ...
	    value: function setEvent(stream) {
	      ...
	      that.ws.on('receiveAnswer', function (desc, id) {
	        ...
	        that.peer[id].setRemoteDescription(desc).then(that.onSetRemoteSuccess.bind(that, id), that.onSetSessionDescriptionError);
	      });
	      ...
	      that.ws.on('receiveIceCandidate', function (candidate, id) {
	        ...
	        that.peer[id].addIceCandidate(new RTCIceCandidate(candidate)).then(that.onAddIceCandidateSuccess, that.onAddIceCandidateError);
	      });
	    }
	  }, {
	    key: 'createOffer',
	    value: function createOffer(id) {

	      console.log('CREATEOFFER');

	      var that = this;

	      that.peer[id].createOffer().then(that.onCreateOfferSuccess.bind(that, id), that.onCreateSessionDescriptionError);
	    }
	  }, {
	    key: 'createPeer',
	    ...
	    value: function onCreateOfferSuccess(id, desc) {
	      ...
	      that.peer[id].setLocalDescription(desc).then(function () {
	    ...
	    value: function onSendChannelStateChange(id) {
	    ...
	    value: function receiveCreateRtc(id) {
	    ...
	    key: 'requestCreateRtc',
	    value: function requestCreateRtc(roomname) {
	    ...
	    value: function onCreateAnswerSuccess(id, desc) {
	      console.log('KKKKKKKKKKKKKKKk--', id, desc);
	      this.peer[id].setLocalDescription(desc).then(this.onSetLocalSuccess.bind(this, desc, id), this.onSetSessionDescriptionError);
	    }
	    ...
	    key: 'onIceCandidate',
	    value: function onIceCandidate(event, id) {
	      if (event.candidate) {
	    ...
	    value: function onSetRemoteSuccess(id) {

	      console.log('SETREMOTEDESCRIPTION COMPLETE');

	      // setremotesdp 성공 후 createSdp 생성
	      if (this.type == 'receiver') {
	        this.peer[id].createAnswer().then(this.onCreateAnswerSuccess.bind(this, id), this.onCreateSessionDescriptionError);
	      }
	    }
	  }, {
